...
const AllAccomodations = (props) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (Array.isArray(data.accommodations)) {
        const newAccommodations = data.accommodations.map(
          (accommodation) => {
            return {
              id: accommodation.id,
        ...
        const filteredAccomodations = newAccommodations.filter((accomodation) => {
          return accomodation.userId === currentUser._id;
        });
  ...
  const accomodationClickHandler = (accomodation) => {
    props.setSelectedAccomodation(accomodation)
          ...
          <table className="game-history-table">
            ...
            <tbody>
              {accomodations.map((accommodation) => (
                <tr onClick={() => accomodationClickHandler(accommodation)} key={accommodation.id}>
